...
|---|---|---|
| Xoboro-native HTTP API | PARTIAL | Setup/session, read-only library/series/media-item discovery, page/resource discovery, and page delivery are implemented; add administration commands, resource byte delivery, file download, artwork, events, cursor pagination, and OpenAPI |
