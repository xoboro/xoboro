...
| Reader web UI | MISSING | Designed in `docs/DESIGN.md`: one Svelte application, two shells, reader ported from simple-komga onto the native API. Remaining: everything after the design — the EPUB and PDF readers are new work rather than a port, the data layer is a rewrite because simple-komga targets the frozen compatibility surface, and serving `web/dist` is an unimplemented server change (no `staticFiles`/`staticResources`/`singlePageApplication` route exists) |
| Administrator web UI | MISSING | Designed in `docs/DESIGN.md`: thirteen screens, each mapped to a native route group that already exists, plus the destructive-action rules for library deletion, trash emptying, task discarding, and duplicate-page decisions. Remaining: the screens themselves |
...
| Artwork upload, discovery, selection, generation, and caching | PARTIAL | Collections and read lists derive a cover from their members: a 2×2 mosaic when four member covers are available, one member's cover otherwise, and the sweep re-derives a cover once the grouping's `updatedAtMillis` passes it, so a membership change is picked up without wiring the three mutation paths (ADR 0098). WebP output is **blocked on a dependency decision** — the JDK ships no WebP `ImageIO` writer, so it needs a new library and has not been added unilaterally |
