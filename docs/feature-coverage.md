...
| Page and Readium locator progress | READY | The mutation/conflict contract is exercised end to end: a newer write applies, an older one is refused with `409 stale_progress` rather than silently rewinding the reader's place, and progress survives a restart (ADR 0101). One known deviation, deliberately not corrected: `totalProgression` is `position / count`, so it reports the progress at the *end* of a position and is one position ahead of the Readium convention. It cannot misplace a reader — resume works from the stored page — but Kobo's `ProgressPercent` is derived from it and is correspondingly high. ADR 0105 records why the fix is a task of its own: stored positions do not migrate themselves |
