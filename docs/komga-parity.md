...
| Page streaming and conversion | PARTIAL | Exact analyzed-entry delivery, EPUB resource streaming, PDF JPEG rendering and raw single-page extraction, raw and zero-based routes, JPEG/PNG conversion, bounded page thumbnails, role and catalog-access enforcement, CSP, resource-lifetime tests, Komga UTF-8 inline names, non-PDF raw rejection, exact shallow ETags, timestamp-first source bypass, HTTP validator precedence, and live byte/ETag/304 certification for inventory, original, JPEG, and thumbnail representations; PDF Accept negotiation and conditional EPUB/resource semantics pending |
