...
| Series aggregation | PARTIAL | Transactional path grouping, book counts, restore/delete, partial-scan safety, ordered metadata providers, lock-preserving merges, and set-based Komga precedence for the first non-blank summary, earliest release date, first-seen distinct authors, tag union, and metadata timestamps with synthetic regression coverage; differential fixtures pending |
