...
|---|---|---|
| Configuration and environment overrides | PARTIAL | Komga-compatible administrator settings GET/PATCH, durable defaults and nullable overrides, explicit null wire fields, environment/database/effective source reporting, restart-applied port and context path, exact configured-origin CORS, trusted proxy and protected metrics configuration, live worker resizing, live remember-me duration and key rotation, strict parsing, and validation tests; full Komga property surface pending |
| SQLite schema and migrations | PARTIAL | Flyway V1-V25 clean-install and V1-to-current upgrade tests |
...
| PDF | PARTIAL | PDFBox page analysis, optional crop-box dimensions, bounded page indexing, JPEG rendering, raw single-page PDF extraction, WebPub PDF manifest, Komga-compatible quality/specificity-based `Accept` negotiation with opt-out, source materialization cleanup, and synthetic document tests; encrypted documents, metadata, and live differential fixtures pending |
| EPUB 2 and EPUB 3 | PARTIAL | Secure ZIP/container/package parsing, manifest/spine resources, fixed-layout and DiViNa detection, EPUB3 navigation plus EPUB2 NCX fallback, Readium positions, KEPUB span detection, package metadata import, persisted extensions, resource delivery with CSP plus timestamp-first source bypass and shallow ETag revalidation, WebPub manifest, bounded KEPUB conversion, and synthetic fixed/reflowable tests; remaining metadata vocabulary and live EPUB differential fixtures pending |
...
| Cover and thumbnail generation | PARTIAL | Durable owner-scoped Book/Series/Collection/ReadList artwork, bounded multipart upload, decoded-pixel safety, JPEG normalization/resizing, atomic selection, uploaded-only deletion, authenticated byte delivery, first-visible-book page fallback, low-priority bulk Book poster generation, atomic local Book/Series sidecar replacement with user-selection preservation, exact owner-specific cache policy, shallow ETags and 304 contracts, plus live byte/cache/ETag certification for selected Book artwork; remaining owner kinds need live differential fixtures |
| Page streaming and conversion | PARTIAL | Exact analyzed-entry delivery, EPUB resource streaming, PDF JPEG rendering and raw single-page extraction, raw and zero-based routes, quality/specificity-based PDF `Accept` negotiation plus `contentNegotiation` opt-out, JPEG/PNG conversion, bounded page thumbnails, role and catalog-access enforcement, CSP, resource-lifetime tests, Komga UTF-8 inline names, non-PDF raw rejection, exact shallow ETags, timestamp-first source bypass, HTTP validator precedence, and live byte/ETag/304 certification for inventory, original, JPEG, and thumbnail representations; live PDF and EPUB differential fixtures pending |
| Book, series, and read-list downloads | PARTIAL | Original book download streams source materialization with Komga UTF-8 attachment naming and single-byte-range responses; original CBZ bytes, type, length, and name are live-differential-certified; Series and ReadList ZIPs stream entries without whole-archive buffering with role/access enforcement and collision-safe names; generated archive metadata differential remains pending |
...
| Capability | Status | Evidence |
|---|---|---|
| ComicInfo.xml import | PARTIAL | Bounded, secure root-entry CBZ parsing implements Komga's complete Book/Series field mapping plus independent `AlternateSeries`/`StoryArc` read-list and `SeriesGroup` collection imports; sparse positions, collisions, case-insensitive matching, idempotency, settings, persistence, durable-job, and runtime behavior have synthetic coverage; differential fixtures pending |
...
| One-shot detection | COMPLETE | Configured-directory, root-Series, and candidate derivation tests plus Komga-compatible first-Book title/summary promotion, ended status, fixed total count, provider precedence, and live Book/Series/filter differential certification |
...
| Series aggregation | PARTIAL | Transactional path grouping, book counts, restore/delete, partial-scan safety, ordered metadata providers, lock-preserving merges, and persisted Komga precedence for the first non-blank summary, earliest release date, first-seen distinct authors, tag union, and metadata timestamps; same-transaction dirty tracking covers metadata, relation, lifecycle, and old/new parent moves, rebuilds in 500-ID batches, survives restart, and powers aggregate filters; differential fixtures pending |
...
| Capability | Status | Evidence |
|---|---|---|
| Series, books, and one-shots | PARTIAL | Upgrade-safe complete scan-state entities, metadata relations, database-paged read model, Komga DTOs, bounded 500-ID batch hydration of parents/metadata/media/progress, stable cross-batch ordering, Komga natural numbering in bounded reconciliation batches, explicit null/source/media flag projection, transactional/integration tests, and live synthetic list/detail/Series Books, successful and boundary sibling navigation, and one-shot Book/Series/filter differential certification; advanced lifecycle differential fixtures pending |
...
| Filtering, sorting, and alphabetical groups | PARTIAL | Every Komga Book and Series sort alias is allowlisted in SQL, with authenticated-user progress, selected read-list/collection manual positions, persisted aggregate release dates, deterministic tie breakers, stable pagination, request-accurate sort envelopes, Komga lowercase alphabetical groups, and bounded recursive `allOf`/`anyOf` conditions across membership, metadata, read/media state, artwork, date/numeric/string/null, and persisted aggregate author/tag/date fields after authorization/content restrictions; deprecated Book/Series compound and case-insensitive title-regex filters plus current structured filters are live-differential-certified against metadata-rich synthetic archives, with remaining work limited to malformed-expression diagnostics and uncommon sort/filter combinations |
...
| Collections and manual ordering | PARTIAL | Durable ordered membership, case-insensitive unique names, transactional CRUD, ComicInfo `SeriesGroup` creation/idempotent extension, access-filtered list/detail/reverse-membership routes with bounded 500-organization visibility hydration, filtered indicators, SQL-paged member authorization/count/order plus Komga metadata/read-state filters, cascade behavior, REST integration tests, and live member-filter plus Series-to-Collections reverse-relation certification; generated thumbnails pending |
| Read lists and manual ordering | PARTIAL | Durable ordered membership and summaries, transactional CRUD, ComicInfo alternate-series/story-arc import with sparse/collision-safe positions, access-filtered list/detail/book/sibling/reverse-membership routes with bounded 500-organization visibility hydration, filtered indicators, SQL-paged member authorization/count/order plus Komga metadata/read/media-state filters, cascade behavior, archive streaming, Mihon progress adapters, REST integration tests, and live member-filter plus Book-to-ReadLists reverse-relation certification |
...
| Capability | Status | Evidence |
|---|---|---|
| Initial administrator claim | PARTIAL | Anonymous GET status is live-differential-certified against Komga 1.25.0; header-based POST claim contracts, atomic single-winner persistence, all-role DTO, TSID identity, Argon2id hashing, null omission, validation, and real-runtime restart tests are covered; imported Komga BCrypt hashes are upgraded after successful authentication; automated initial-user configuration and authenticated differential claim fixtures pending |
| Multi-user CRUD and password reset | PARTIAL | Komga v2 current-user, admin list/create/patch/delete, self/admin password change, duplicate/validation/not-found behavior, durable relations, immediate credential replacement tests, and exact timestamped Spring error envelopes for explicit failures; demo mode pending |
...
| Basic authentication | COMPLETE | Ktor's Basic credential parser plus a bounded OPDS challenge adapter preserve case-insensitive success and multi-provider session/API-key/remember-me behavior while matching Komga's quoted realm, timestamped REST/OPDS v1 error document, OPDS v2 authentication document and discovery link, and missing/malformed/unknown/wrong credential behavior; unit, integration, and live differential tests cover every challenge form |
| API keys | PARTIAL | Ktor's API-key authentication provider validates `X-API-Key`; dashless UUID generation, TSID IDs, ten-attempt collision handling, Komga SHA-512 storage, redacted list, duplicate comment, owner deletion, cascade, and authentication tests are covered; demo mode and protocol-specific key transports pending |
...
| R2 locator and EPUB progression | PARTIAL | Durable per-user Readium locator/device/timestamp state, stale-write conflict rejection, authenticated GET/PUT contracts, persisted EPUB positions, position-list contract, Komga non-empty WebPub serialization and comic export media types, conditional manifest/position ETags, DiViNa/EPUB integration tests, and live DiViNa manifest plus 304 certification; EPUB/PDF manifests and advanced locator differential fixtures pending |
...
| Previous and next book semantics | PARTIAL | Stable metadata number, relative-path, and ID tie-breaking in SQL with access checks, unit tests, and live Komga-exact previous/next boundary error certification; successful multi-book traversal and read-state semantics pending |
...
| Capability | Status | Evidence |
|---|---|---|
| Komga REST API v1/v2 | PARTIAL | All 165 pinned operations have an implementation in progress, including management info, font resources, release discovery, durable poster regeneration, atomic duplicate-page removal, a post-serialization shallow ETag filter across successful byte-array GET responses with exact Komga exclusions, bounded transient/duplicate-image revalidation, exact context-aware Spring security-chain response headers, timestamped 401/403/404/500 envelopes, and 400 deserialization classification; remaining schemas and route-specific error diagnostics need differential completion |
| Komga authentication and session semantics | PARTIAL | Basic, `X-API-Key`, `KOMGA-SESSION`, `X-Auth-Token`, and `komga-remember-me` authentication, multi-provider principal propagation, seven-day inactivity, 365-day remember-me restoration, transport conversion, logout, credential deletion, password-signature invalidation, security-change invalidation, and live-differential-certified configured CORS tests; OAuth pending |
...
| OPDS v1 | COMPLETE | All 19 Atom/OpenSearch catalog, browse, acquisition, thumbnail, page, and wildcard-download operations use access-filtered catalog data; Komga-exact valid Atom/OpenSearch structure, feed/entry metadata, filtering, paging links, navigation/acquisition/PSE links, organization ordering, zero-based page translation, exact page/thumbnail ETags, timestamp-first source bypass, UTF-8 dispositions, archive acquisition, timestamped missing-resource errors, and exact unauthorized JSON/Basic challenges are synthetic-tested and live-differential-certified |
| OPDS v2 and authentication document | PARTIAL | All 30 feed, browse, search, collection/read-list/series detail, artwork, page, profile manifest, authentication-document, progression, and OPDS acquisition operations reuse shared catalog/WebPub/progress/media boundaries; exact paging, subsection navigation, compact publications, organization ordering/access, publisher and tag facets, summary fallback, acquisition streaming, one-based page delivery, feed ETag emission, page/thumbnail revalidation, full authentication document challenges, and missing-resource envelopes are synthetic-tested and live-differential-certified; advanced relevance remains |
| Kobo Sync and KEPUB | PARTIAL | All 15 URL-token routes, role enforcement, initialization, device auth, access-filtered metadata/state/shelves, restart-safe incremental SQLite snapshots and continuation cursors, conditionally cached thumbnails, EPUB downloads, bounded external kepubify conversion with revision cache invalidation, no-proxy catch-all behavior, and exact deny-frame security headers have synthetic integration coverage; Kobo Store proxy merging and differential device fixtures pending |
