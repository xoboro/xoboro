...
| SQLite schema and migrations | PARTIAL | Flyway V1-V23 clean-install and V1-to-current upgrade tests |
...
| Structured logs, health, readiness, metrics | PARTIAL | Worker failure logging, `/health` liveness, SQLite-backed `/ready`, and token-protected bounded Prometheus request/readiness/queue/worker/uptime metrics with no path or identifier labels; structured JSON fields pending |
